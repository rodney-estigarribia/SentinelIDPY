  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-4 space-y-4">
        <h3 className="font-bold text-white text-base">Plantillas Maestras ({templates.length})</h3>
        ...
        <div className="p-4 rounded-xl border border-slate-800 bg-slate-900/40 space-y-2.5 text-xs">
          ...
          <div className="flex items-start gap-2 text-slate-300">
            ...
            <span>
              <strong>Base de Datos (`wp_options`)</strong>: 100% automatizable vía API REST remota sin tocar código.
            </span>
          </div>
          <div className="flex items-start gap-2 text-slate-300">
            <span className="w-2 h-2 rounded-full bg-amber-400 mt-1 shrink-0" />
            <span>
              <strong>Archivos de Disco (`.htaccess`, `wp-config.php`)</strong>: Aplicable con permisos de escritura de servidor.
            </span>
          </div>
          <div className="flex items-start gap-2 text-slate-300">
            <span className="w-2 h-2 rounded-full bg-rose-400 mt-1 shrink-0" />
            <span>
              <strong>Ajustes Manuales</strong>: Parámetros de terceros con guía de pasos.
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="lg:col-span-8 space-y-6">
        ...
        <div className="p-5 rounded-xl border border-slate-800 bg-slate-900/60 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-white text-sm">Comparativa de Configuración (Drift)</h3>
              <p className="text-xs text-slate-400">
                Analizando plantilla: <span className="text-white font-semibold">{currentTemplate.name}</span>
              </p>
            </div>
          </div>
        ...
        >
          ...
          {totalDrift > 0 && (
            <button
              ...
              disabled={isApplying}
              className="px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold transition-colors disabled:opacity-50 whitespace-nowrap shadow-md"
            >
        ...
        {syncSuccess && (
          <div className="p-3.5 rounded-lg bg-emerald-950 border border-emerald-500/30 text-xs text-emerald-300 font-semibold">
              ...
              {driftItems.map((item) => (
                <tr
                  ...
                  className={`transition-colors ${
                    item.isDrift ? 'bg-amber-500/5 hover:bg-amber-500/10' : 'hover:bg-slate-800/20'
                  }`}
                >
                  <td className="py-3 px-4 font-mono font-bold text-white">{item.key}</td>
                  <td className="py-3 px-4 font-mono text-emerald-400">{item.templateValue}</td>
                  <td
                    className={`py-3 px-4 font-mono font-semibold ${
                      item.isDrift ? 'text-amber-400' : 'text-slate-300'
                    }`}
                  >
                    {item.clientValue}
                  </td>
