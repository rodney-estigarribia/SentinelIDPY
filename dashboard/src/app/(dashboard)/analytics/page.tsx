  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen">
      ...
      <div className="p-6 space-y-6 max-w-7xl mx-auto w-full">
        {/* Top Summary Banner */}
        <div className="p-5 rounded-xl border border-cyan-500/20 bg-gradient-to-r from-cyan-950/30 to-slate-900/60 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h3 className="font-bold text-white text-base">Arquitectura de Analítica Ultraligera</h3>
            <p className="text-xs text-slate-300 mt-1 max-w-2xl leading-relaxed">
              Los hits de visitas se registran de forma aislada en la base de datos de cada cliente WordPress en la tabla{' '}
              <code className="bg-slate-950 px-1.5 py-0.5 rounded text-cyan-300 font-mono">wp_sentinel_analytics</code> con limpieza automática semestral. Tu panel central solo consulta resúmenes consolidados, protegiendo al 100% el límite gratuito de Vercel y Neon.
            </p>
          </div>

          <div className="text-right">
            <span className="text-xs text-slate-400 block">Total Visitas (Semestre)</span>
            <span className="text-2xl font-extrabold text-cyan-400 tracking-tight">124,580</span>
          </div>
        </div>
        ...
        <div className="p-6 rounded-xl border border-slate-800 bg-slate-900/40 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h4 className="font-bold text-white text-sm">Tendencia Global de Audiencia (Abril - Septiembre)</h4>
              <p className="text-xs text-slate-400">Visitas combinadas en todos los sitios de la cartera</p>
            </div>
          ...
          <div className="h-56 flex items-end justify-between gap-4 pt-8 px-4 bg-slate-950/70 border border-slate-800 rounded-xl">
            {[
              ...
              <div key={m.month} className="flex-1 flex flex-col items-center gap-2 h-full justify-end group">
                <span className="text-xs font-mono font-bold text-cyan-400 opacity-0 group-hover:opacity-100 transition-opacity">
                  {m.visits}
                </span>
                <div
                  className="w-full bg-cyan-500/30 hover:bg-cyan-500/70 border border-cyan-500/50 rounded-t-md transition-all duration-300"
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 overflow-hidden">
          <div className="p-4 border-b border-slate-800">
            <h4 className="font-bold text-white text-sm">Desglose por Sitio</h4>
          </div>
